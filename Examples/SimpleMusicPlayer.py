...
class SimpleMusicPlayer(GnuChanGUI):
    def __init__(self, Title = "Music Player", Size = (1600, 900), resizable = False, finalize = True, winPosX = 1920 / 2, winPosY = 1080 / 2):
    ...
    def MusicPlay(self):
        ...
        elif self.GetEvent == "Next Music" or self.numpad1 == self.CurrentKey:
            if not self.start:
                _musicName = self.musicPlay.NextSound_SingleChannel()
                self.GetWindow["musicName"].update(self.musicPlay.MusicName)

        elif self.GetEvent == "Previous Music" or self.numpad2 == self.CurrentKey:
            if not self.start:
